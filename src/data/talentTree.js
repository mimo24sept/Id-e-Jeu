...
const TALENT_CENTER_NODES = [
  ...
  { id: "ct-1",      x: -75,  y: 130,  tier: 2, cost: 25, name: "Endurance",    desc: "+8% PV max",                region: "center", effect: { maxHpMultiplier: 0.08 },             connections: [] },
  ...
  { id: "ct-5",      x: 106,  y: -106, tier: 2, cost: 25, name: "Tactique",     desc: "+5% dégâts, +5% PV max",    region: "center", effect: { damage: 0.05, maxHpMultiplier: 0.05 }, connections: [] },
  ...
  { id: "ct-8",      x: 80,   y: -80,  tier: 1, cost: 18, name: "Vitalité",     desc: "+5% PV max, +0.3% PV/s",   region: "center", effect: { maxHpMultiplier: 0.05, regenPercent: 0.003 }, connections: [] },
...
const TALENT_SPADES = buildRegionNodes("spades", -300, 0, -1, 0, [
  ...
  [1, 1,1,18,"Impact brut",        "+2% dégâts",                 {damage:0.02}],
  [1, 2,1,20,"Lame lourde",        "+2% dégâts",                 {damage:0.02}],
  ...
  [2, 1,1,18,"Blessure profonde",  "+2% dégâts",                 {damage:0.02}],
  ...
  [4, 1,2,35,"Art de la lame",     "+7% dégâts",                 {damage:0.07}],
  [4, 2,2,38,"Coup imparable",     "+4% dégâts",                 {damage:0.04}],
  [5,-1,2,42,"Savoir-faire",       "+5% dégâts, +2% crit",       {damage:0.05,critChance:0.02}],
  [5, 0,2,42,"Perfection guerrière","+7% dégâts",                {damage:0.07}],
  [5, 1,2,42,"Frappe implacable",  "+8% dégâts",                 {damage:0.08}],
  ...
  [6, 1,2,45,"Impact dévastateur", "+5% dégâts",                 {damage:0.05}],
  [6,-2,3,80,"Assassin",           "+12% dégâts, +10% crit",    {damage:0.12,critChance:0.10}],
  [6, 2,3,80,"Lame vorace",        "+15% dégâts, -0.4% PV/s",  {damage:0.15,regenPercent:-0.004}],
  [7, 0,3,90,"Transcendance",      "+14% dégâts, +8% crit",     {damage:0.14,critChance:0.08}],
  [7,-1,3,85,"Chasseur d'élites",  "+12% crit, +8% dégâts",     {critChance:0.12,damage:0.08}],
  [7, 1,3,85,"Poing de fer",       "+12% dégâts",                {damage:0.12}],
  ...
  [8, 1,3,90,"Furie dévastatrice", "+15% dégâts",               {damage:0.15}],
  ...
  [10,-1,2,55,"Force brute ultime","+12% dégâts",                {damage:0.12}],
...
const TALENT_HEARTS = buildRegionNodes("hearts", -150, 260, -0.5, 0.866, [
  [1,-2,1,18,"Chair robuste",      "+4% PV max",                 {maxHpMultiplier:0.04}],
  [1,-1,1,18,"Résistance",         "+4% PV max",                 {maxHpMultiplier:0.04}],
  [1, 0,1,18,"Corps solide",       "+4% PV max",                 {maxHpMultiplier:0.04}],
  [1, 1,1,18,"Récupération",       "+0.3% PV/s",                 {regenPercent:0.003}],
  [1, 2,1,18,"Endurance vitale",   "+0.3% PV/s",                 {regenPercent:0.003}],
  [2,-2,1,20,"Fortitude",          "+4% PV max",                 {maxHpMultiplier:0.04}],
  [2,-1,1,18,"Vigueur",            "+4% PV max",                 {maxHpMultiplier:0.04}],
  [2, 0,1,18,"Peau dure",          "+4% PV max",                 {maxHpMultiplier:0.04}],
  [2, 1,1,18,"Régénération",       "+0.3% PV/s",                 {regenPercent:0.003}],
  [2, 2,1,20,"Vitalité pure",      "+4% PV max",                 {maxHpMultiplier:0.04}],
  [3,-2,2,35,"Muraille de chair",  "+10% PV max",                {maxHpMultiplier:0.10}],
  [3,-1,1,20,"Force vitale",       "+5% PV max",                 {maxHpMultiplier:0.05}],
  [3, 0,2,35,"Résilience",         "+10% PV max",                {maxHpMultiplier:0.10}],
  [3, 1,1,20,"Flux sanguin",       "+0.4% PV/s",                 {regenPercent:0.004}],
  [3, 2,2,38,"Guérison rapide",    "+0.6% PV/s",                 {regenPercent:0.006}],
  [4,-2,2,38,"Cuirasse naturelle", "+6% PV max",                 {maxHpMultiplier:0.06}],
  [4,-1,2,35,"Puits de vie",       "+8% PV max, +0.4% PV/s",    {maxHpMultiplier:0.08,regenPercent:0.004}],
  [4, 0,2,40,"Titan",              "+12% PV max",                {maxHpMultiplier:0.12}],
  [4, 1,2,35,"Flux vital",         "+0.5% PV/s, +5% PV max",    {regenPercent:0.005,maxHpMultiplier:0.05}],
  [4, 2,2,38,"Régénération soutenue","+0.7% PV/s",              {regenPercent:0.007}],
  [5,-1,2,42,"Force vitale majeure","+13% PV max",              {maxHpMultiplier:0.13}],
  [5, 0,2,42,"Constitution légendaire","+13% PV max",           {maxHpMultiplier:0.13}],
  [5, 1,2,42,"Sang de guerre",     "+0.6% PV/s, +6% PV max",   {regenPercent:0.006,maxHpMultiplier:0.06}],
  [5,-2,3,75,"Colosse",            "+20% PV max, -12% vitesse att.",{maxHpMultiplier:0.20,attackSpeed:-0.12}],
  [5, 2,3,75,"Flux inépuisable",   "+1.2% PV/s, -8% dégâts",   {regenPercent:0.012,damage:-0.08}],
  [6,-1,2,45,"Corps de titan",     "+15% PV max",               {maxHpMultiplier:0.15}],
  [6, 0,2,45,"Invulnérabilité",    "+14% PV max",               {maxHpMultiplier:0.14}],
  [6, 1,2,45,"Métabolisme actif",  "+0.8% PV/s, +5% PV max",   {regenPercent:0.008,maxHpMultiplier:0.05}],
  [6,-2,3,80,"Forteresse vivante", "+14% PV max, +0.8% PV/s",  {maxHpMultiplier:0.14,regenPercent:0.008}],
  [6, 2,3,80,"Sang immortel",      "+1.5% PV/s",                {regenPercent:0.015}],
  [7, 0,3,90,"Apothéose",          "+18% PV max, +0.8% PV/s",  {maxHpMultiplier:0.18,regenPercent:0.008}],
  [7,-1,3,85,"Citadelle",          "+16% PV max",               {maxHpMultiplier:0.16}],
  [7, 1,3,85,"Régénération totale","+1.2% PV/s, +8% PV max",   {regenPercent:0.012,maxHpMultiplier:0.08}],
  [8, 0,3,95,"Immortel",           "+18% PV max, +1% PV/s",    {maxHpMultiplier:0.18,regenPercent:0.010}],
  [8,-1,3,90,"Titan de guerre",    "+22% PV max",               {maxHpMultiplier:0.22}],
  [8, 1,3,90,"Régénérant",         "+1.5% PV/s",                {regenPercent:0.015}],
  [9,-1,2,50,"Vitalité transcendante","+0.8% PV/s, +8% PV max",{regenPercent:0.008,maxHpMultiplier:0.08}],
  [9, 0,3,92,"Sang régénérant",    "+25% soins reçus",            {healingBonus:0.25}],
  [9, 1,2,50,"Bouclier vital",     "+6% PV max",                  {maxHpMultiplier:0.06}],
  [10,-1,2,55,"Flux éternel",      "+1.2% PV/s, +10% PV max",    {regenPercent:0.012,maxHpMultiplier:0.10}],
  [10, 0,3,95,"Résurrection partielle","+45% soins reçus",        {healingBonus:0.45}],
  [10, 1,2,55,"Corps indestructible","+20% PV max",               {maxHpMultiplier:0.20}],
...
const TALENT_DIAMONDS = buildRegionNodes("diamonds", 150, 260, 0.5, 0.866, [
  ...
  [9, 0,3,92,"Intérêts composés",  "+2% intérêts fin de vague",  {interestBonus:0.02}],
  ...
  [10, 0,3,95,"Capitaliste absolu","+3% intérêts fin de vague",  {interestBonus:0.03}],
  [10, 1,2,55,"Fonds d'urgence",   "+10% fragments, +8% or",     {fragmentGain:0.10,money:0.08}],
  [11, 0,3,98,"Seigneur des marchés","+4% intérêts fin de vague, +20% or",{interestBonus:0.04,money:0.20}],
]);
...
const TALENT_CLUBS = buildRegionNodes("clubs", 300, 0, 1, 0, [
  ...
  [1, 1,1,18,"Agilité",            "+2.5% dépl.",                {moveSpeedMultiplier:0.025}],
  [1, 2,1,18,"Légèreté",           "+2.5% dépl.",                {moveSpeedMultiplier:0.025}],
  ...
  [2, 1,1,18,"Accélération",       "+2.5% dépl.",                {moveSpeedMultiplier:0.025}],
  ...
  [3, 1,1,20,"Sprint",             "+5% dépl.",                  {moveSpeedMultiplier:0.05}],
  [3, 2,2,38,"Dash constant",      "+7% dépl.",                  {moveSpeedMultiplier:0.07}],
  [4,-2,2,38,"Rafale",             "+5% vitesse att.",           {attackSpeed:0.05}],
  [4,-1,2,35,"Tir continu",        "+4% vitesse att., +3% dépl.",{attackSpeed:0.04,moveSpeedMultiplier:0.03}],
  [4, 0,2,40,"Tempête de plombs",  "+6% vitesse att.",           {attackSpeed:0.06}],
  [4, 1,2,35,"Vent de guerre",     "+4% vitesse att., +5% dépl.",{attackSpeed:0.04,moveSpeedMultiplier:0.05}],
  [4, 2,2,38,"Vitesse pure",       "+8% dépl.",                  {moveSpeedMultiplier:0.08}],
  [5,-1,2,42,"Mitraille",          "+5% vitesse att., +3% dépl.",{attackSpeed:0.05,moveSpeedMultiplier:0.03}],
  [5, 0,2,42,"Déluge",             "+7% vitesse att.",           {attackSpeed:0.07}],
  [5, 1,2,42,"Vent déchaîné",      "+5% vitesse att., +6% dépl.",{attackSpeed:0.05,moveSpeedMultiplier:0.06}],
  [5,-2,3,75,"Furie",              "+18% vitesse att., -10% dégâts",{attackSpeed:0.18,damage:-0.10}],
  [5, 2,3,75,"Fantôme",            "+18% dépl.",                 {moveSpeedMultiplier:0.18}],
  [6,-1,2,45,"Cadence infernale",  "+6% vitesse att., +4% dépl.",{attackSpeed:0.06,moveSpeedMultiplier:0.04}],
  [6, 0,2,45,"Foudre",             "+8% vitesse att.",           {attackSpeed:0.08}],
  [6, 1,2,45,"Vent du nord",       "+9% dépl.",                  {moveSpeedMultiplier:0.09}],
  [6,-2,3,80,"Tornade",            "+15% vitesse att., +8% dépl.",{attackSpeed:0.15,moveSpeedMultiplier:0.08}],
  [6, 2,3,80,"Éclair",             "+12% dépl., +5% vitesse att.",{moveSpeedMultiplier:0.12,attackSpeed:0.05}],
  [7, 0,3,90,"Tempête absolue",    "+10% vitesse att., +8% dépl.",{attackSpeed:0.10,moveSpeedMultiplier:0.08}],
  [7,-1,3,85,"Déchaînement",       "+15% vitesse att.",          {attackSpeed:0.15}],
  [7, 1,3,85,"Vent éternel",       "+14% dépl.",                 {moveSpeedMultiplier:0.14}],
  [8, 0,3,95,"Dieu de la vitesse", "+20% vitesse att., +12% dépl.",{attackSpeed:0.20,moveSpeedMultiplier:0.12}],
  [8,-1,3,90,"Avalanche",          "+18% vitesse att.",          {attackSpeed:0.18}],
  [8, 1,3,90,"Vrille",             "+16% dépl.",                 {moveSpeedMultiplier:0.16}],
  [9,-1,2,50,"Frappe rapide",      "+10% vitesse att., +5% dépl.",{attackSpeed:0.10,moveSpeedMultiplier:0.05}],
  [9, 0,3,92,"Premier ricochet",   "+1 rebond de projectile",    {bounceCount:1}],
  [9, 1,2,50,"Vent vif",           "+8% vitesse att., +7% dépl.",{attackSpeed:0.08,moveSpeedMultiplier:0.07}],
  ...
  [10, 1,2,55,"Tempête intérieure","+10% vitesse att., +10% dépl.",{attackSpeed:0.10,moveSpeedMultiplier:0.10}],
...
const TALENT_POKER = buildRegionNodes("poker", 0, -300, 0, -1, [
  ...
  [1, 1,1,18,"Joker de poche",     "+4% PV max",                 {maxHpMultiplier:0.04}],
  ...
  [2, 0,1,18,"Paire gagnante",     "+1.5% dégâts, +0.2% PV/s",  {damage:0.015,regenPercent:0.002}],
  [2, 1,1,18,"Double mise",        "+4% PV max, +1.5% or",      {maxHpMultiplier:0.04,money:0.015}],
  ...
  [4, 0,2,40,"Main pleine",        "+3% dégâts, +6% PV, +2% vitesse",{damage:0.03,maxHpMultiplier:0.06,attackSpeed:0.02}],
  [4, 1,2,35,"Pot commun",         "+3% or, +6% PV",            {money:0.03,maxHpMultiplier:0.06}],
  ...
  [5, 1,2,42,"Jackpot",            "+4% or, +8% PV max",        {money:0.04,maxHpMultiplier:0.08}],
  ...
  [6, 1,2,45,"Roi du casino",      "+5% or, +6% PV max",        {money:0.05,maxHpMultiplier:0.06}],
...
const TALENT_COMBAT = buildRegionNodes("combat", 212, -212, 0.707, -0.707, [
  ...
  [1, 2,1,18,"Chargeur lourd",     "+2% dégâts",                 {damage:0.02}],
  ...
  [2, 2,1,20,"Munitions lourdes",  "+2% dégâts",                 {damage:0.02}],
  ...
  [3, 2,2,38,"Impact cinétique",   "+4% dégâts",                 {damage:0.04}],
  ...
  [4, 2,2,38,"Obus perforant",     "+5% dégâts",                 {damage:0.05}],
  ...
  [6, 1,2,45,"Feux croisés",       "+5% vitesse att., +6% dégâts",{attackSpeed:0.05,damage:0.06}],
  [6,-2,3,80,"Sniper légendaire",  "+12% crit, +10% dégâts",    {critChance:0.12,damage:0.10}],
  [6, 2,3,80,"Avalanche de plomb", "+10% dégâts, +10% vitesse att.",{damage:0.10,attackSpeed:0.10}],
  ...
  [8, 1,3,90,"Tempête de feu",     "+12% dégâts, +12% vitesse att.",{damage:0.12,attackSpeed:0.12}],
  ...
  [9, 1,2,50,"Arsenal perfectionné","+8% dégâts, +6% vitesse att.",{damage:0.08,attackSpeed:0.06}],
  ...
  [10, 1,2,55,"Armure balistique", "+10% dégâts, +8% vitesse att.",{damage:0.10,attackSpeed:0.08}],
...
const TALENT_GUERRIERS = buildRegionNodes("guerriers", 572, 330, 0.866, 0.5, [
  [1, 0, 2, 50, "Rage guerrière",        "+6% dégâts",                                {damage:0.06}],
  ...
  [2, 1, 2, 55, "Obus du Bazooka",       "+10% dégâts",                               {damage:0.10}],
  ...
  [3, 1, 3, 75, "Canon",                 "+14% dégâts",                               {damage:0.14}],
  [4,-1, 3, 85, "Taille noire",          "+12% dégâts, +10% crit",                   {damage:0.12, critChance:0.10}],
  [4, 0, 3, 85, "Rage totale",           "+12% dégâts, +10% vitesse att.",           {damage:0.12, attackSpeed:0.10}],
  [5, 0, 3, 95, "Incarnation guerrière", "+20% dégâts, +10% crit",                   {damage:0.20, critChance:0.10}],
]);

// ✚ VITAUX — Ange Blanc · Moine · Vampire — 90° (entre Carreau et Cœur)
const TALENT_VITAUX = buildRegionNodes("vitaux", 0, 660, 0, 1, [
  [1, 0, 2, 50, "Vitalité profonde",     "+10% PV max, +0.5% PV/s",                  {maxHpMultiplier:0.10, regenPercent:0.005}],
  [2,-1, 2, 55, "Grâce de l'Ange",       "+0.8% PV/s, +8% PV max",                   {regenPercent:0.008, maxHpMultiplier:0.08}],
  [2, 0, 2, 55, "Sang du Vampire",       "+25% soins reçus, +8% PV max",             {healingBonus:0.25, maxHpMultiplier:0.08}],
  [2, 1, 2, 55, "Sanctuaire du Moine",   "+12% PV max, +0.6% PV/s",                  {maxHpMultiplier:0.12, regenPercent:0.006}],
  [3,-1, 3, 75, "Flux vital",            "+1% PV/s, +12% PV max",                    {regenPercent:0.010, maxHpMultiplier:0.12}],
  [3, 0, 3, 75, "Immortalité",           "+35% soins reçus, +15% PV max",            {healingBonus:0.35, maxHpMultiplier:0.15}],
  [3, 1, 3, 75, "Corps de titan",        "+16% PV max, +1% PV/s",                    {maxHpMultiplier:0.16, regenPercent:0.010}],
  [4,-1, 3, 85, "Régén absolue",         "+1.5% PV/s, +10% PV max",                  {regenPercent:0.015, maxHpMultiplier:0.10}],
  [4, 0, 3, 85, "Sang vital",            "+45% soins reçus, +12% PV max",            {healingBonus:0.45, maxHpMultiplier:0.12}],
  [5, 0, 3, 95, "Immortel vivant",       "+50% soins, +18% PV max, +1.5% PV/s",     {healingBonus:0.50, maxHpMultiplier:0.18, regenPercent:0.015}],
]);
...
const TALENT_CAMELEONS = buildRegionNodes("cameleons", -572, 330, -0.866, 0.5, [
  [1, 0, 2, 50, "Polyvalence",           "+4% dégâts, +5% PV max, +4% vitesse att.", {damage:0.04, maxHpMultiplier:0.05, attackSpeed:0.04}],
  [2,-1, 2, 55, "Instinct du terrain",   "+5% dégâts, +8% dépl.",                   {damage:0.05, moveSpeedMultiplier:0.08}],
  [2, 0, 2, 55, "Équilibre du CHAD",     "+5% dégâts, +6% PV max, +5% vitesse att.",{damage:0.05, maxHpMultiplier:0.06, attackSpeed:0.05}],
  [2, 1, 2, 55, "Croissance lente",      "+0.8% dégâts par vague, +6% PV max",      {waveScaling:0.008, maxHpMultiplier:0.06}],
  [3,-1, 3, 75, "Adaptation",            "+8% dégâts, +10% PV max, +8% dépl.",      {damage:0.08, maxHpMultiplier:0.10, moveSpeedMultiplier:0.08}],
  [3, 0, 3, 75, "Transcendance",         "+10% dégâts, +1.2% par vague, +8% PV max",{damage:0.10, waveScaling:0.012, maxHpMultiplier:0.08}],
  [3, 1, 3, 75, "Collection infinie",    "+6% dégâts, +12% PV max, +5% vitesse att.",{damage:0.06, maxHpMultiplier:0.12, attackSpeed:0.05}],
  [4,-1, 3, 85, "Instinct pur",          "+10% dégâts, +10% dépl., +6% PV max",     {damage:0.10, moveSpeedMultiplier:0.10, maxHpMultiplier:0.06}],
  [4, 0, 3, 85, "Évolution tardive",     "+12% dégâts, +1.5% par vague",             {damage:0.12, waveScaling:0.015}],
  [5, 0, 3, 95, "Caméléon légendaire",   "+15% dégâts, +10% PV max, +2% par vague", {damage:0.15, maxHpMultiplier:0.10, waveScaling:0.020}],
]);
...
const TALENT_NEGOCIANTS = buildRegionNodes("negociants", -610, -252, -0.924, -0.383, [
  ...
  [2,-1, 2, 55, "Intérêts du Banquier",  "+2% intérêts fin de vague",                 {interestBonus:0.02}],
  [2, 0, 2, 55, "Commerce maîtrisé",     "+8% or, -5% packs, +3% frag",              {money:0.08, packDiscount:0.05, fragmentGain:0.03}],
  [2, 1, 2, 55, "Revente premium",       "+12% revente cartes",                        {sellBonus:0.12}],
  [3,-1, 3, 75, "Usure du Banquier",     "+4% intérêts fin de vague",                 {interestBonus:0.04}],
  [3, 0, 3, 75, "Fortune absolue",       "+15% or, -8% packs, +6% frag",             {money:0.15, packDiscount:0.08, fragmentGain:0.06}],
  [3, 1, 3, 75, "Liquidation",           "+18% revente cartes, +6% or",              {sellBonus:0.18, money:0.06}],
  [4,-1, 3, 85, "Rentier",               "+5% intérêts fin de vague",                 {interestBonus:0.05}],
  [4, 0, 3, 85, "Monopole",              "+18% or, -10% packs, +8% frag",            {money:0.18, packDiscount:0.10, fragmentGain:0.08}],
  [5, 0, 3, 95, "Seigneur marchand",     "+25% or, +22% revente, -12% packs",         {money:0.25, sellBonus:0.22, packDiscount:0.12}],
]);
...
const TALENT_RAPIDES = buildRegionNodes("rapides", 610, -252, 0.924, -0.383, [
  [1, 0, 2, 50, "Cadence d'enfer",       "+7% vitesse att., +5% dépl.",               {attackSpeed:0.07, moveSpeedMultiplier:0.05}],
  [2,-1, 2, 55, "Rafale de la Tempête",  "+8% vitesse att., +1 rebond",               {attackSpeed:0.08, bounceCount:1}],
  [2, 0, 2, 55, "Chronos brisé",         "+6% vitesse att., +6% dépl.",               {attackSpeed:0.06, moveSpeedMultiplier:0.06}],
  [2, 1, 2, 55, "Vent perpétuel",        "+8% dépl., +6% vitesse att.",               {moveSpeedMultiplier:0.08, attackSpeed:0.06}],
  [3,-1, 3, 75, "Maître du tempo",       "+12% vitesse att., +2 rebonds",              {attackSpeed:0.12, bounceCount:2}],
  [3, 0, 3, 75, "Fusillade divine",      "+14% vitesse att., +10% dépl.",             {attackSpeed:0.14, moveSpeedMultiplier:0.10}],
  [3, 1, 3, 75, "Fantôme",               "+12% dépl., +8% vitesse att.",              {moveSpeedMultiplier:0.12, attackSpeed:0.08}],
  [4,-1, 3, 85, "Ricochet maîtrisé",     "+10% vitesse att., +2 rebonds",              {attackSpeed:0.10, bounceCount:2}],
  [4, 0, 3, 85, "Overdrive",             "+18% vitesse att., +12% dépl.",             {attackSpeed:0.18, moveSpeedMultiplier:0.12}],
  [5, 0, 3, 95, "Tireur légendaire",     "+22% vitesse att., +3 rebonds, +8% dépl.",  {attackSpeed:0.22, bounceCount:3, moveSpeedMultiplier:0.08}],
]);
...
const TALENT_INTER_NODES = [
  ...
  { id:"br-1a", x:242,  y:140,  tier:2, cost:35, region:"bridge", connections:[],
    name:"Ruée dévastatrice",   desc:"+4% vitesse att., +5% dépl.", effect:{attackSpeed:0.04, moveSpeedMultiplier:0.05} },
  { id:"br-1b", x:329,  y:190,  tier:2, cost:45, region:"bridge", connections:[],
    name:"Torrent de vitesse",  desc:"+5% vitesse att., +7% dépl.", effect:{attackSpeed:0.05, moveSpeedMultiplier:0.07} },
  { id:"br-1c", x:416,  y:240,  tier:3, cost:65, region:"bridge", connections:[],
    name:"Ouragan",             desc:"+10% vitesse att., +12% dépl.", effect:{attackSpeed:0.10, moveSpeedMultiplier:0.12} },

  // Zone 2 — 90° (Diamonds ↔ Hearts)
  { id:"br-2a", x:0,    y:280,  tier:2, cost:35, region:"bridge", connections:[],
    name:"Capital vital",       desc:"+5% or, +6% PV max",          effect:{money:0.05, maxHpMultiplier:0.06} },
  { id:"br-2b", x:0,    y:380,  tier:2, cost:45, region:"bridge", connections:[],
    name:"Fortune et santé",    desc:"+7% or, +9% PV max",          effect:{money:0.07, maxHpMultiplier:0.09} },
  { id:"br-2c", x:0,    y:480,  tier:3, cost:65, region:"bridge", connections:[],
    name:"Baron prospère",      desc:"+12% or, +14% PV max",        effect:{money:0.12, maxHpMultiplier:0.14} },

  // Zone 3 — 150° (Hearts ↔ Spades)
  { id:"br-3a", x:-242, y:140,  tier:2, cost:35, region:"bridge", connections:[],
    name:"Frappe résistante",   desc:"+4% dégâts, +6% PV max",      effect:{damage:0.04, maxHpMultiplier:0.06} },
  { id:"br-3b", x:-329, y:190,  tier:2, cost:45, region:"bridge", connections:[],
    name:"Gladiateur",          desc:"+5% dégâts, +9% PV max",      effect:{damage:0.05, maxHpMultiplier:0.09} },
  { id:"br-3c", x:-416, y:240,  tier:3, cost:65, region:"bridge", connections:[],
    name:"Champion",            desc:"+10% dégâts, +14% PV max",    effect:{damage:0.10, maxHpMultiplier:0.14} },
  ...
  { id:"br-7a", x:-259, y:-107, tier:2, cost:35, region:"bridge", connections:[],
    name:"Commerce sanglant",   desc:"+4% dégâts, +8% revente cartes", effect:{damage:0.04, sellBonus:0.08} },
  { id:"br-7c", x:-443, y:-184, tier:3, cost:65, region:"bridge", connections:[],
    name:"Assassin marchand",   desc:"+8% dégâts, +10% revente cartes", effect:{damage:0.08, sellBonus:0.10} },
];
